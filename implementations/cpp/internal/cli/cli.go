...
func Run(invocation Invocation) int {
	...
	files, err := collectCppFiles(selection)
	if err != nil {
		fmt.Fprintf(invocation.Stderr, "vet: %v\n", err)
		return 2
	}
	...
	if cfg.GithubActionsPinned.Enabled {
		...
		for _, file := range workflowFiles {
			...
			fileDiagnostics, err := analyzer.AnalyzeWorkflowFile(cppanalysis.AnalyzeWorkflowFileRequest{
				Path:   file,
				Source: source,
			})
			if err != nil {
...
func collectCppFiles(request fileCollectionRequest) ([]string, error) {
...
func (c *fileCollection) addFile(path string) {
	if !isCppSourceFile(path) || c.seenFiles[path] || c.isExcluded(path) {
		return
	}
...
func isCppSourceFile(path string) bool {
	...
	case ".c", ".h", ".cc", ".cpp", ".cxx", ".c++", ".hh", ".hpp", ".hxx", ".h++", ".ipp", ".tpp", ".inl":
		return true
	default:
		return false
	}
...
func shouldSkipDir(name string) bool {
	switch name {
	case ".git", "vendor", "node_modules", "build", "cmake-build-debug", "cmake-build-release", "out", "dist", "target":
		return true
	default:
